...
Production runtime commit is `0d825fb`, which is deployed and verified on production. The next operations step is external review execution: send or reassign the 25 overdue beta invites, send the due-soon visual-review request for `PROD-VISUAL-HISTORY-004`, record sent proof with `qa/dispatch-sent-record-template-2026-06-05.csv`, validate/import that proof, then collect completed beta and visual review JSON evidence.
