...
The newest release-candidate coverage checkpoint moves that multi-itinerary public-share Browser loop into the orchestrated predeploy gate. `QA_RELEASE_INCLUDE_SHARE_MULTI_ITINERARY=1 npm run qa:release-candidate` now runs `qa:share-multi-itinerary-ui` as part of the local launch-candidate matrix. Focused validation passed `18/18`; the multi-itinerary task passed `37/37` with ten disposable fixtures, social-card image checks, recipient feedback flows, owner feedback readback, feedback refresh, and cleanup. Evidence: `qa/release-candidate-share-multi-integration-2026-05-21/README.md`.
