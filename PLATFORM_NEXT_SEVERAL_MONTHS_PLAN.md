...
The newest release-operations hardening closes a same-day evidence ambiguity in launch signoff. `npm run qa:launch-signoff` now requires postdeploy production evidence to include the live production commit from `/api/health` or `QA_LAUNCH_EXPECTED_COMMIT`, and its default evidence artifact points at the current production-evidence record. The strengthened signoff passed `32/32`; the older same-day release note now fails as expected because it lacks current commit `c2933cd0211cb12c05bcb09096298b505faa926e`. Production public-share viral Browser smoke also passed `5/5` with no app error or horizontal overflow. Commit `6d1b7f7` deployed to Vercel production with health `ok`, `11/11`, and the non-visual production release gate passed `9/9`; the evidence record now includes that deployed commit for current-commit signoff. Evidence: `qa/launch-signoff-current-production-evidence-2026-05-21.md`.
