...
Current signoff checkpoint: `npm run qa:launch-signoff` now provides the fast evidence audit for release meetings. It passed `27/27` on 2026-05-21, proving production health `11/11`, full local release-candidate evidence `33/33`, responsive visual QA `50/50`, hosted Stripe evidence, launch docs, fresh postdeploy production evidence, current launch risk register, and current rollback plan. Use `QA_LAUNCH_EXPECTED_COMMIT=<sha>` when a launch meeting needs the audit to prove a specific production commit is live.
