...
Deployment refresh: production is healthy on Vercel commit `e6294047034a653f45f489cfa5d0c6e45d601bcf` at deployment `globe-travel-oftzu4ak3-rodney-blairs-projects.vercel.app`, and the latest production visual evidence remains `qa/visual-baseline-production-runtime-current-2026-05-22-e629404` plus `qa/visual-baseline-production-runtime-current-2026-05-22-e629404/summary.json`; however production is not current for runtime source because commit `6966b711bbcc60589205e20316c0ff77aae052e6` (`6966b71`) is still waiting on deploy. The refreshed production release gate `QA_BASE_URL=https://globe-travel-two.vercel.app QA_SHARE_SLUG=x3m2c8cnws QA_PRODUCTION_VISUAL_ARTIFACT_NAME=visual-baseline-production-runtime-current-2026-05-22-e629404 npm run qa:release-production` passed `10/10`, production visual QA passed `25/25`, production ops with required public metadata passed `3/3`, Athens public share/map integrity passed `5/5`, prompt-suite validation with the Athens production actual passed `60/60`, and production monitoring passed `9/9`. Runtime source commit `16b30eb4255da0dd7acedcca1a1432ec258bc4ee` replaces the stale internal `AlbatrossBrand` primitive with `GlobeBrand`, updates the assistive label to `Globe.travel logo mark`, and adds a design-system stale-brand guard; commit `e6294047034a653f45f489cfa5d0c6e45d601bcf` adds the launch-status deployment-currency guard so stale production runtime source blocks beta-ready status. `npm run qa:public-launch-status` now reports `blocked`, runtime deployment current `no`, and guardrail issue `production is behind runtime commit 6966b71`; `npm run qa:launch-signoff` currently fails `119/120` until production catches up to runtime commit `6966b71`. Local guest-surface verification after the runtime fix passed `npm run qa:app-surfaces` `22/22`, `npm run qa:auth-access` `15/15`, `npm run lint`, and `npm run build` before the current Vercel quota block. The remaining public-launch blockers are deployment currency, beta reviews `0/25`, `25 remaining`, current wave `BETA-WAVE-01`, and production visual-review history `2/4` with `2 remaining`. The active evidence queues remain `qa/beta-human-review-next-wave-ops-2026-05-21.json`, `qa/beta-human-review-all-wave-ops-2026-05-21.json`, `qa/beta-human-review-follow-up-outbox-2026-05-21.json`, `qa/beta-human-review-wave-rehearsal-2026-05-22.json`, `qa/beta-human-review-matrix-rehearsal-2026-05-22.json`, `qa/public-launch-blocker-board-2026-05-21.json`, and the operator sent-record starter `qa/dispatch-sent-record-template-2026-05-22.json`, `qa/dispatch-sent-record-template-2026-05-22.md`, and `qa/dispatch-sent-record-template-2026-05-22.csv`. `npm run qa:launch-today` remains the guarded daily operator board and now includes the P0 runtime deployment action for commit `6966b71` alongside the 5 due beta invites and 1 due visual-review assignment; `npm run qa:dispatch-sent-record-template` now produces one non-import-ready, privacy-safe sent-state starter for the 6 outreach rows, `npm run qa:dispatch-sent-record-template-rejection` writes `qa/dispatch-sent-record-template-rejection-2026-05-22.json` plus `qa/dispatch-sent-record-template-rejection-2026-05-22.md` to prove the blank starter fails in import mode without mutating canonical dispatch logs, `npm run qa:review-intake-import-rehearsal` writes `qa/review-intake-import-rehearsal-2026-05-22.json` plus `qa/review-intake-import-rehearsal-2026-05-22.md` to prove valid completed beta and visual-review submissions can import into copied registers while canonical launch evidence remains unchanged, and `npm run qa:public-launch-threshold-rehearsal` writes `qa/public-launch-threshold-rehearsal-2026-05-22.json` plus `qa/public-launch-threshold-rehearsal-2026-05-22.md` to prove copied complete registers make the beta and visual threshold gates report ready. Launch signoff verifies those artifacts alongside the mark-sent dry run and isolated import rehearsal. Production route inventory passes `22/22`, production app surfaces pass `22/22`, production guest accessibility/keyboard passes `22/22`, design-system readiness passes `11/11`, and `/pricing` remains a public acquisition surface with `Globe.travel pricing`, `Start 7-day free trial`, and `Adventurer`.
