...
Deployment refresh: launch social preview and public metadata routes are live on Vercel production commit `98413c3f43be80be1db4ed8b4bae4469eeb633f7` at deployment `globe-travel-oa7q4lzn2-rodney-blairs-projects.vercel.app`; the refreshed production release gate `QA_BASE_URL=https://globe-travel-two.vercel.app QA_SHARE_SLUG=x3m2c8cnws QA_PRODUCTION_VISUAL_ARTIFACT_NAME=visual-baseline-production-social-preview-2026-05-22-98413c3 npm run qa:release-production` passed `10/10`, production visual QA passed `25/25`, public metadata smoke passed `6/6`, production ops with required public metadata passed `3/3`, prompt-suite validation with the Athens production actual passed `60/60`, and production monitoring passed `9/9`. Runtime source commit `16b30eb4255da0dd7acedcca1a1432ec258bc4ee` now replaces the stale internal `AlbatrossBrand` primitive with `GlobeBrand`, updates the assistive label to `Globe.travel logo mark`, and adds a design-system stale-brand guard, but the production deploy is pending because `vercel deploy --prod --yes` hit `api-deployments-free-per-day`. `npm run qa:public-launch-status` now correctly reports `blocked` with guardrail issue `production is behind runtime commit 16b30eb` until that runtime source reaches production. The latest deployed production visual evidence remains `qa/visual-baseline-production-social-preview-2026-05-22-98413c3` and `qa/visual-baseline-production-social-preview-2026-05-22-98413c3/summary.json`. The real public-launch blockers are still beta reviews `0/25`, `25 remaining`, current wave `BETA-WAVE-01`, and production visual-review history `2/4` with `2 remaining`. The active evidence queues remain `qa/beta-human-review-next-wave-ops-2026-05-21.json`, `qa/beta-human-review-all-wave-ops-2026-05-21.json`, `qa/beta-human-review-follow-up-outbox-2026-05-21.json`, `qa/beta-human-review-wave-rehearsal-2026-05-22.json`, `qa/beta-human-review-matrix-rehearsal-2026-05-22.json`, and `qa/public-launch-blocker-board-2026-05-21.json`. `npm run qa:launch-today` remains the guarded daily operator board, production route inventory passes `22/22`, production app surfaces pass `22/22`, production guest accessibility/keyboard passes `22/22`, design-system readiness passes `11/11`, and `/pricing` remains a public acquisition surface with `Globe.travel pricing`, `Start 7-day free trial`, and `Adventurer`.
