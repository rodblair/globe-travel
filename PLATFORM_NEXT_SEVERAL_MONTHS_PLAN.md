...
Deployment refresh: production is healthy on Vercel commit `79bcaf47c17abc6338d3f2a725c79c2d3ac1f3fa` at deployment `globe-travel-mbbrhizi0-rodney-blairs-projects.vercel.app`, and the latest current-production visual evidence is `qa/visual-baseline-production-runtime-current-2026-05-23-79bcaf4` plus `qa/visual-baseline-production-runtime-current-2026-05-23-79bcaf4/summary.json`, bound to the same commit and deployment. The refreshed current-deployment production release gate `QA_BASE_URL=https://globe-travel-two.vercel.app QA_SHARE_SLUG=x3m2c8cnws QA_PRODUCTION_VISUAL_ARTIFACT_NAME=visual-baseline-production-runtime-current-2026-05-23-79bcaf4 npm run qa:release-production` passed `10/10`, production visual QA passed `25/25`, production ops with required public metadata passed `3/3`, Athens public share/map integrity passed `5/5`, public share viral loop passed `5/5`, prompt-suite validation with the Athens production actual passed `60/60`, production auth and guest access passed `13/13`, production Trip Studio recovery UI passed `1/1`, design-system readiness passed `11/11`, production monitoring passed `9/9`, and the malformed guest-cookie regression passed without a protected API crash. `npm run qa:public-launch-status` now reports status `beta-ready-public-blocked` with no guardrail issues, beta reviews `0/25`, `25 remaining`, current wave `BETA-WAVE-01`, and production visual-review history `2/4` with `2 remaining`. The active evidence queues remain `qa/beta-human-review-next-wave-ops-2026-05-21.json`, `qa/beta-human-review-all-wave-ops-2026-05-21.json`, `qa/beta-human-review-follow-up-outbox-2026-05-21.json`, `qa/beta-human-review-wave-rehearsal-2026-05-22.json`, `qa/beta-human-review-matrix-rehearsal-2026-05-22.json`, `qa/public-launch-blocker-board-2026-05-21.json`, and the operator sent-record starter `qa/dispatch-sent-record-template-2026-05-23.json`, `qa/dispatch-sent-record-template-2026-05-23.md`, and `qa/dispatch-sent-record-template-2026-05-23.csv`. `npm run qa:launch-refresh` now passes `10/10` while tolerating 2 actionable launch-board outreach steps; `npm run qa:launch-today` remains the guarded daily operator board with 5 overdue beta invite sends and 1 due visual-review assignment, and exposes exact sent-record CSV handoff commands: `QA_DISPATCH_MARK_SENT_RECORD=qa/dispatch-sent-record-template-2026-05-23.csv npm run qa:dispatch-mark-sent`, `QA_DISPATCH_MARK_SENT_IMPORT=1 QA_DISPATCH_MARK_SENT_RECORD=qa/dispatch-sent-record-template-2026-05-23.csv npm run qa:dispatch-mark-sent`, then `npm run qa:launch-refresh` and `npm run qa:launch-signoff`. `npm run qa:dispatch-sent-record-template` passes `10/10`, while `npm run qa:dispatch-sent-record-template-rejection`, `npm run qa:review-intake-import-rehearsal`, `npm run qa:public-launch-threshold-rehearsal`, `npm run qa:dispatch-mark-sent-import-rehearsal`, and `npm run qa:public-launch-mode-rehearsal` continue to prove sent-state, import, threshold, and mode behavior without mutating canonical launch evidence. Current proof artifacts include `qa/dispatch-sent-record-template-rejection-2026-05-23.json`, `qa/dispatch-sent-record-template-rejection-2026-05-23.md`, `qa/review-intake-import-rehearsal-2026-05-23.json`, `qa/review-intake-import-rehearsal-2026-05-23.md`, `qa/public-launch-threshold-rehearsal-2026-05-23.json`, and `qa/public-launch-threshold-rehearsal-2026-05-23.md`. Production route inventory passes `22/22`, production app surfaces pass `22/22`, production guest accessibility/keyboard passes `22/22`, and `/pricing` remains a public acquisition surface with `Globe.travel pricing`, `Start 7-day free trial`, and `Adventurer`.
...
The next public-share catalog hardening closes the larger production-data issue behind the single Athens check. `npm run qa:share-map-catalog` now discovers live public shares from Supabase and validates every discovered public itinerary/map link against production. The first catalog run found 5 public shares: Athens `x3m2c8cnws` passed, one stale QA fixture was still public, and 3 older itinerary links had duplicate mapped-stop or missing-route issues. `npm run qa:share-catalog-quarantine` then set the 4 non-launch-ready links private without deleting their trips, and the latest rerun now discovers exactly 1 public share, stable Athens `x3m2c8cnws`, with 1/1 shares, 2/2 phone/desktop viewports, 5/5 itinerary days, usable routes, unique mapped stops, Greece-only labels, share controls, feedback surfaces, no app error, and no horizontal overflow. Public launch status and launch signoff now default to `qa/public-share-map-itinerary-integrity-2026-05-23.json` and require live catalog discovery, so a hidden broken public itinerary cannot pass release readiness just because the stable Athens URL works.
