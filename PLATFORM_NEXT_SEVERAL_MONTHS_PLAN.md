...
Deployment refresh: production is healthy on Vercel commit `c0484fdf992b56fb161785b7127ed4a4cdf0694f` at deployment `globe-travel-ofpoq84o2-rodney-blairs-projects.vercel.app`, and the latest current-production visual evidence is `qa/visual-baseline-production-runtime-current-2026-05-23-c0484fd` plus `qa/visual-baseline-production-runtime-current-2026-05-23-c0484fd/summary.json`, bound to the same commit and deployment. The refreshed current-deployment production release gate `QA_BASE_URL=https://globe-travel-two.vercel.app QA_SHARE_SLUG=x3m2c8cnws QA_PRODUCTION_VISUAL_ARTIFACT_NAME=visual-baseline-production-runtime-current-2026-05-23-c0484fd npm run qa:release-production` passed `10/10`, production visual QA passed `25/25`, production ops with required public metadata passed `3/3`, Athens public share/map integrity passed `5/5`, prompt-suite validation with the Athens production actual passed `60/60`, production auth and guest access passed `13/13`, and the malformed guest-cookie regression passed without a protected API crash. The latest pushed runtime commit is `b4948d57bc6ff6ef2f9a364730ee0d1940a62e9f` (`b4948d5`), which adds the mobile skip-link leak fix, but production has not deployed it yet; `npm run qa:public-launch-status` now reports `production is behind runtime commit b4948d5`. The Vercel automatic deployment for `b4948d5`, `globe-travel-p3jd932ur-rodney-blairs-projects.vercel.app`, was canceled, and direct production deploy is currently blocked by daily quota `api-deployments-free-per-day`. `npm run qa:launch-refresh` is the routine operator refresh command; it regenerates the daily launch board and public-launch status together, and the current evidence reports runtime deployment blocked, beta reviews `0/25`, `25 remaining`, current wave `BETA-WAVE-01`, and production visual-review history `2/4` with `2 remaining`. The active evidence queues remain `qa/beta-human-review-next-wave-ops-2026-05-21.json`, `qa/beta-human-review-all-wave-ops-2026-05-21.json`, `qa/beta-human-review-follow-up-outbox-2026-05-21.json`, `qa/beta-human-review-wave-rehearsal-2026-05-22.json`, `qa/beta-human-review-matrix-rehearsal-2026-05-22.json`, `qa/public-launch-blocker-board-2026-05-21.json`, and the operator sent-record starter `qa/dispatch-sent-record-template-2026-05-22.json`, `qa/dispatch-sent-record-template-2026-05-22.md`, and `qa/dispatch-sent-record-template-2026-05-22.csv`. `npm run qa:launch-today` remains the guarded daily operator board with 1 P0 production deployment row, 5 due beta invites, and 1 due visual-review assignment, while `npm run qa:dispatch-sent-record-template-rejection`, `npm run qa:review-intake-import-rehearsal`, and `npm run qa:public-launch-threshold-rehearsal` continue to prove sent-state, import, and threshold behavior without mutating canonical launch evidence. Production route inventory passes `22/22`, production app surfaces pass `22/22`, production guest accessibility/keyboard passes `22/22`, design-system readiness passes `11/11`, and `/pricing` remains a public acquisition surface with `Globe.travel pricing`, `Start 7-day free trial`, and `Adventurer`.
