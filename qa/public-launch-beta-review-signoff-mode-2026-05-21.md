...
- Default exact-commit signoff passed `75/75` for beta/release-candidate operations, including `25/25` beta reviewer packet coverage, beta review progress artifact consistency, paid-path readiness `6/6`, and the `3/3` future production visual-review schedule.
- `QA_LAUNCH_REQUIRE_PUBLIC_BETA_REVIEWS=1 QA_LAUNCH_EXPECTED_COMMIT=ec38399177df6919d31e128baab202ad74444679 npm run qa:launch-signoff` fails as expected at `75/77`, with `0/25` completed beta reviews and `1/4` required production visual-review history entries.
