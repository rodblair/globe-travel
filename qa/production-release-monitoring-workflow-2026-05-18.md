...
- Manual inputs can override:
  ...
  - production viral-loop QA inclusion
...
Production viral-loop QA is enabled by default and should only be disabled with `include_production_viral=0` when isolating browser share API or clipboard infrastructure issues. Remote guest-start mutation remains skipped by the viral smoke unless `QA_ALLOW_REMOTE_GUEST_MUTATION=1` is explicitly set outside the scheduled workflow.
...
- `QA_BASE_URL=https://globe-travel-two.vercel.app QA_SHARE_SLUG=x3m2c8cnws QA_PRODUCTION_VISUAL_ARTIFACT_NAME=visual-baseline-production-release-2026-05-18 npm run qa:release-production` passed `9/9`, including public production viral-loop QA `5/5`, public production visual QA `20/20`, and prompt-suite validation `52/52`.
...
It now also catches public visual regressions and viral-loop regressions in acquisition, auth conversion, and public share surfaces as part of the same release signal.
