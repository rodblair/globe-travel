...
const date = requestedDate || currentQaDate()
...
const betaDispatchDueToday = betaRows
  .filter((row) => daysBetween(today, row.sendBy) === 0)
  .map((row) => betaActionRow(row, 'P0', 'Send beta review invite today.'))
const betaDispatchOverdue = betaRows
  .filter((row) => Number.isFinite(daysBetween(today, row.sendBy)) && daysBetween(today, row.sendBy) < 0)
...
const visualDueSoon = visualRows
  .filter((row) => {
    const delta = daysBetween(today, row.dueAt)
    return row.status === 'required for public launch history' && Number.isFinite(delta) && delta >= 0 && delta <= 7
  })
  .map((row) => visualActionRow(row, 'P1', 'Schedule reviewer time and prepare production visual-review run.'))
const visualOverdue = visualRows
  .filter((row) => {
    const delta = daysBetween(today, row.dueAt)
    return row.status === 'required for public launch history' && Number.isFinite(delta) && delta < 0
  })
...
function actionRowsTable(rows) {
  if (!rows.length) return '| none | none | none | none | none | none | none |\n'
  return rows.map((row) => `| ${row.priority} | ${row.workType} | ${row.id} | ${row.dueAt || 'n/a'} | ${row.action} | \`${row.messageFile || row.startUrlOrCommand || 'n/a'}\` | \`${row.submissionPath || 'n/a'}\` |`).join('\n')
}
