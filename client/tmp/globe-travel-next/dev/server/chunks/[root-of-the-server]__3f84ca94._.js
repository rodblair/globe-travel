...
"[project]/app/api/user-places/route.ts [app-route] (ecmascript)", ((__turbopack_context__) => {
"use strict";
...
async function GET() {
    ...
    const { data, error } = await supabase.from('user_places').select('*, place:places(*)').eq('user_id', user.id).order('created_at', {
        ascending: false
    });
    if (error) {
        return __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$next$2f$server$2e$js__$5b$app$2d$route$5d$__$28$ecmascript$29$__["NextResponse"].json({
...
async function POST(request) {
    ...
    const { data: placeData, error: placeError } = await supabase.from('places').upsert({
        name: place.name,
    ...
    const { data: userPlace, error: upError } = await supabase.from('user_places').insert({
        user_id: user.id,
    ...
    return __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$next$2f$server$2e$js__$5b$app$2d$route$5d$__$28$ecmascript$29$__["NextResponse"].json(userPlace, {
        status: 201
    });
}
}),
...
//# sourceMappingURL=%5Broot-of-the-server%5D__3f84ca94._.js.map